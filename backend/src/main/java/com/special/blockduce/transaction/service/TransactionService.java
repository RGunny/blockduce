...
public class TransactionService {
    ...
    public void create(DbcEthDto form) {
        ...
        if(form.getIsDbcEth()==1) {
            ...
            if(member.getId()!=null){ //계정이 있을 경우
                member.updateEth(member.getEth() - form.getValue());
            }
        }
    }
   ...
   @Transactional
    public DbcEthDto findMemberByid(Long id) {
       ...
       System.out.println("member의 어카운트 찾아왔니? :"+member.get().getAccount());
       DbcEthDto dbcDto = new DbcEthDto();
        if(member.isPresent()){ //계정이 있을 경우
            dbcDto =  DbcEthDto.builder().
                    senderAccount(member.get().getAccount()).
                    build();
        }
        return dbcDto;
    }
    ...
    public DbcEthDto findCandidateByid(Long id) {
        ...
        System.out.println("candidate 어카운트 찾아왔니? :"+candidate.get().getAccount());

        DbcEthDto dbcDto = new DbcEthDto();
        if(candidate.isPresent()){ //계정이 있을 경우
            dbcDto =  DbcEthDto.builder().
                    receiverAccount(candidate.get().getAccount()).
                    build();
        }
        return dbcDto;
    }
