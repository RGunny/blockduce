...
userRouter.post('/login', (req, res) => {
  ...
  if (!newUser.username)
    return res.send({ code: 400, message: 'Data is required' });
  console.log(`Login user ${newUser.username}`);

  ChatRedis.getUser(newUser.username, config.KEY).then((user) => {
    if (!user) {
...
userRouter.post('/logout', (req, res) => {
  ...
  ChatRedis.delUser(user.username, config.KEY).then((data) => {
